...
if (isset($_POST['hai'])) {
    ...
    $id_dap_an = $_SESSION['id_dap_an_d7k1'];
    ...
    foreach ($arr as $key => $value) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            $sql_insert = "INSERT INTO `dap_an_nguoi_dung_d7k1`(`id_dap_an_nguoi_dung_d7k1`, `id_dap_an`, `dap_an`, `stt_huong_dan`, `id_bai_tap_user`,`luu_vet`) VALUES 
...
function get_dap_an($id_dap_an,$dap_an){
    global $conn;
    $sql = "SELECT * FROM `dap_an_d7k1` WHERE `id_dap_an_d7k1`='$id_dap_an' AND `ten_dap_an`='$dap_an'";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        return 1; 
    } 
...
function check($last_id){
    global $conn;
    $sql="SELECT * FROM `dap_an_nguoi_dung_d7k1` WHERE `id_dap_an_nguoi_dung_d7k1`='$last_id'";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        $id_bai_tap_user = $row['id_bai_tap_user'];
