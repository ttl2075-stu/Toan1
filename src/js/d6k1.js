...
let lesson = JSON.parse(localStorage.getItem('d6k1_baiDien')) || []
let quizShow = ''
...
else if (lesson[0].kiemTra == 'cotrudiem') divNotice.innerHTML = 'Trả lời nhiều lần, trả nời sai sẽ trừ 20% số điểm của câu đấy, tối đa 3 lần sai'
else divNotice.innerHTML = 'Trả lời nhiều lần, tối đa 3 lần sai'
...
function show() {
    lesson.forEach((e, index) => {
                                ...
                                <h1><deBai>${lesson[index].cauHoi}</deBai></h1>
                                    <center>
                                        <div class="box-phepTinh">
                                                <div align="right" class="box-num" id="box-num1">${arrNum[0]}</div>
                                                <div align="left" class="box-num" id="box-pheptinh">${arrNum[1]}</div>
                                                <div align="right" class="box-num" id="box-num2">${arrNum[2]}</div>
                                                <hr>
                                                </div>
                                                <div class="dien" id="dien${index}" style="display: flex; justify-content: center; align-items: center;" >
                                                    <button id="incrementButton">&nbsp;<i class="fa-solid fa-angle-up"></i></button>
                                                    <input type="number" name="checkA" id="checkA${index}"  class="checkA inputStyle1" style="width: 100px; margin: 5px 10px;" >
        ...
        let arrTags = [parseInt(e.dapAn)]
        let max = parseInt(e.dapAn)+4
        console.log(arrTags);
        if (Tags.length != 4) {
            for (let i = 0; i < 4; i++) {
                let str = (i == ranNum) ? `<div class="tag" id="the${index}_${i}" draggable="true" ondragstart="drag(event)">${e.dapAn}</div>` :
                    `<div class="tag" id="the${index}_${i}" draggable="true" ondragstart="drag(event)">${getRandomNumberNotInArray(arrTags,max)}</div>`
                tags.innerHTML += str
            }
        }
    });
...
function checkAnswer() {
    ...
    inputs.forEach((input, index) => {
        ...
        if (index == quizShow) {
            ...
            if (lesson[index].soLanTraLoiSai == 3 && lesson[index].kiemTra == 'cotrudiem') {
                lesson[index].diem = 0
                khoaInput(input, index)
            }
...
function khoaInput(input, index) {
    ...
    if (lesson[index].soLanTraLoiSai == 3) document.querySelector(`#notice${index}`).textContent = 'Bạn đã trả lời sai 3 lần, đáp án là ' + lesson[index].dapAn
}
...
function showDien() {
    ...
    if (lesson[quizShow].mucHelp > 1) document.querySelector('.dien').classList.add('hidden')
